...
import healpy as hp
from Tmax_helperfunctions import Q_measure
...
def getDipoleVectors_healpy(densitymap, mask=[None], galcut=0, verbose=False) :
	...
	residual,monopole,dipole = hp.remove_dipole(densitymap,bad=np.nan,fitval=True,gal_cut=galcut)
	norm = np.sqrt(np.dot(dipole,dipole))
	dipole_norm = dipole/norm
...
def doAll_Vectors_resampling(N, N_output, rot_mat, rot_mat_inv, rot_mat_mask, rot_mat_mask_inv, maskangles, alpha, flux, x=1., galcut=30., seed=123, vel=velocity_CMBframe, do_resampling=True,W1_fluxcut=W1_fluxcut, add_isotropic=0.,estimator='linear',nside=32,masking='symmetric', mask=[None], weights=[None]) :
	...
	if estimator=='linear' :
		...
		# theta= -theta+np.pi/2
		densitymap1,d = hp.remove_monopole(densitymap,gal_cut=30,fitval=True)
		densitymap2 = densitymap1[unmask]
		phi = phi[unmask]
		theta = theta[unmask]
		df_Qdata = Q_measure(densitymap2,theta,phi)
		Tmax = df_Qdata[df_Qdata['delta_Qd'] == df_Qdata['delta_Qd'].max()]	
		Tmax_sel = Tmax
		print(len(Tmax))
		if len(Tmax) > 1 :
			Tmax_sel = Tmax[Tmax.theta2>np.pi/2]
			print(Tmax_sel)
	
		vec = hp.ang2vec(theta=Tmax_sel.theta2,phi=Tmax_sel.phi2)
		print(vec)
		return vec,None,Tmax_sel.theta2,Tmax_sel.phi2
